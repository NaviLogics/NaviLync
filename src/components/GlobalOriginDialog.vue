...
const saveGlobalOrigin = (): void => {
  ...
  if (
    ...
    openSnackbar({ message: t('globalOrigin.offline'), variant: 'error' })
    return
  }
  ...
  try {
    ...
    openSnackbar({ message: t('globalOrigin.success'), variant: 'success' })
    emit('origin-set', latitude.value, longitude.value) // eslint-disable-line
    closeDialog()
  } catch (error) {
    openSnackbar({ message: t('globalOrigin.failed', { error }), variant: 'error' })
  } finally {
...
const dialogActions = computed<Action[]>(() => [
  ...
  {
    text: isSaving.value ? t('common.saving') : t('common.save'),
