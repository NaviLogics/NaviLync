          ...
          <v-menu v-if="widgetStore.editingMode" theme="dark">
            ...
            <v-list>
              <v-list-item @click="handleOptionClick('options')">
                <v-list-item-title>{{ $t('widgetConfig.collapsibleContainer.options') }}</v-list-item-title>
              </v-list-item>
              <v-list-item @click="handleOptionClick('save')">
                <v-list-item-title>{{ $t('widgetConfig.collapsibleContainer.save') }}</v-list-item-title>
              </v-list-item>
              <v-list-item @click="handleOptionClick('load')">
                <v-list-item-title>{{ $t('widgetConfig.collapsibleContainer.load') }}</v-list-item-title>
              </v-list-item>
              <v-list-item @click="handleOptionClick('copy-to-view')">
                <v-list-item-title>{{ $t('widgetConfig.collapsibleContainer.cloneToView') }}</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-menu>
        </div>
    ...
    <v-dialog v-model="widgetStore.widgetManagerVars(widget.hash).configMenuOpen" persistent>
      <GlassModal :is-visible="widgetStore.widgetManagerVars(widget.hash).configMenuOpen">
        <v-card class="px-8 pb-6 pt-2 rounded-lg w-[400px] bg-transparent">
          <v-card-title class="text-center -mt-1">{{ $t('widgetConfig.collapsibleContainer.title') }}</v-card-title>
          <v-btn
          ...
          <div class="flex flex-col justify-start items-start gap-x-4 mt-3 w-full">
            <p class="text-start">{{ $t('widgetConfig.collapsibleContainer.name') }}</p>
            <v-text-field ref="nameInput" v-model="widget.name" density="compact" class="w-3/4" />
            <p class="text-start">{{ $t('widgetConfig.collapsibleContainer.columns') }}</p>
            <v-text-field
              ref="nameInput"
              ...
              class="w-1/4"
            />
            <p class="mt-1">{{ $t('widgetConfig.collapsibleContainer.backgroundColor') }}</p>
            <input v-model="widget.options.backgroundColor" type="color" class="p-0 w-20 mr-4" />
            <p class="mt-3">{{ $t('widgetConfig.collapsibleContainer.backgroundOpacity') }}</p>
            <v-slider
              ...
              width="250"
            />
            <p class="mt-3">{{ $t('widgetConfig.collapsibleContainer.backgroundBlur') }}</p>
      ...
      <GlassModal :is-visible="selectViewToShareDialog">
        <v-card class="px-3 pb-6 pt-2 rounded-lg w-auto bg-transparent z-40">
          <v-card-title class="flex justify-around -mt-1 w-full px-0">
            <div />
            <p class="mx-8">{{ $t('widgetConfig.collapsibleContainer.cloneWidgetTo') }}</p>
...
const handleCopyWidgetToView = (): void => {
  if (selectedViewToShareWidget.value) {
    try {
      ...
      openSnackbar({
        variant: 'success',
        message: t('widgetConfig.collapsibleContainer.widgetCloned'),
      })
    } catch (error: any) {
      openSnackbar({
        variant: 'error',
        message: error.message || t('widgetConfig.collapsibleContainer.errorCloning'),
      })
    }
...
const loadWidget = (event: Event): void => {
  ...
  if (!file) {
    openSnackbar({ variant: 'error', message: t('widgetConfig.collapsibleContainer.noFileSelected'), duration: 3000 })
    return
  }
  ...
  reader.onload = (e) => {
    try {
      ...
      openSnackbar({ variant: 'error', message: t('widgetConfig.collapsibleContainer.invalidFormat'), duration: 3000 })
    }
  }
...
const loadWidgetFromStore = (): void => {
  try {
    ...
    openSnackbar({ variant: 'warning', message: t('widgetConfig.collapsibleContainer.errorReading'), duration: 1000 })
  }
}
