  ...
  <div class="w-full h-full">
    ...
    <v-dialog v-model="widgetStore.widgetManagerVars(widget.hash).configMenuOpen" min-width="600" max-width="45%">
      <v-card class="pa-2" :style="interfaceStore.globalGlassMenuStyles">
        <v-card-title class="text-center">{{ $t('widgetConfig.iframe.title') }}</v-card-title>
        <v-card-text>
          <div>
            <p>{{ $t('widgetConfig.iframe.iframeSource') }}</p>
          ...
          <div class="mt-2 mb-2 w-[95%]">
            <v-slider
              ...
              :label="$t('widgetConfig.iframe.transparency')"
              color="white"
              :min="0"
              :max="90"
            />
          ...
          <ExpansiblePanel compact :is-expanded="true" no-bottom-divider no-top-divider>
            <template #title>{{ $t('widgetConfig.iframe.advancedOptions') }}</template>
            <template #content>
              <v-switch
                v-model="widget.options.useVehicleAddressAsBase"
                :label="$t('widgetConfig.iframe.useVehicleAddress')"
                color="white"
                density="compact"
                hide-details
                class="ml-3 my-2"
              ...
              <div class="flex justify-between">
                <v-switch
                  ...
                  :label="$t('widgetConfig.iframe.collapsibleContainer')"
                  color="white"
                  class="ml-2"
                />
                <div v-if="widget.options.isCollapsible">
                  <v-text-field
                    ...
                    :label="$t('widgetConfig.iframe.containerName')"
                    item-title="name"
                    density="compact"
                    variant="outlined"
                    no-data-text="iframe"
...
const validateURL = (url: string): true | string => {
  return isValidURL(url) ? true : t('widgetConfig.iframe.urlNotValid')
}
...
const updateURL = (): void => {
  ...
  if (urlValidationResult !== true) {
    openSnackbar({ message: `${urlValidationResult} ${t('widgetConfig.iframe.pleaseEnterValid')}`, variant: 'error' })
    return
  }
  widget.value.options.source = inputURL.value
  openSnackbar({ message: t('widgetConfig.iframe.urlUpdated', { url: toBeUsedURL.value }), variant: 'success' })
}
