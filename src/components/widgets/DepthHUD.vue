<template>
  ...
  <v-dialog v-model="widgetStore.widgetManagerVars(widget.hash).configMenuOpen" min-width="400" max-width="35%">
    <v-card class="pa-2" :style="interfaceStore.globalGlassMenuStyles">
      <v-card-title class="text-center">{{ $t('widgetConfig.depthHud.title') }}</v-card-title>
      <v-card-text>
        <v-switch
          class="ma-1"
          :label="$t('widgetConfig.depthHud.showHeightValue')"
        ...
        <v-expansion-panels theme="dark">
          <v-expansion-panel class="bg-[#FFFFFF11] text-white">
            <v-expansion-panel-title>{{ $t('widgetConfig.depthHud.color') }}</v-expansion-panel-title>
