    ...
    <v-card class="rounded-lg" :style="interfaceStore.globalGlassMenuStyles">
      ...
      <v-container>
        <div class="flex flex-col h-[70vh] border border-[rgba(255,255,255,0.3)] rounded elevation-3">
          ...
          >
            ...
            <v-tooltip location="top" :text="$t('settingsManager.saveConfig')">
              <template #activator="{ props: tooltipProps }">
      ...
      <v-card-actions>
        <div class="flex justify-between items-center p-2 w-full h-full text-[rgba(255,255,255,0.5)]">
          <v-btn @click="resetAllCockpitSettings">{{ $t('settingsManager.resetToDefaults') }}</v-btn>
...
const finishInlineJsonEditing = (key: string): void => {
  try {
    ...
    openSnackbar({
      message: t('settingsManager.invalidJson', { error: error.message }),
      variant: 'error',
      duration: 5000,
    })
  }
    ...
    reader.onload = (e: ProgressEvent<FileReader>) => {
      try {
        ...
        if (!json || typeof json !== 'object') {
          openSnackbar({
            message: t('settingsManager.invalidConfigFile'),
            variant: 'warning',
            duration: 5000,
          })
          return
        ...
        openSnackbar({ message: t('settingsManager.configApplied'), variant: 'success', duration: 5000 })
      } catch (error: any) {
        console.error('Error parsing configuration file: ' + error.message)
        openSnackbar({
          message: t('settingsManager.errorParsingConfig', { error: error.message }),
          variant: 'error',
          duration: 5000,
        })
      }
...
const resetAllCockpitSettings = (): void => {
  showDialog({
    message: t('settingsManager.resetConfirmMessage'),
    variant: 'warning',
    maxWidth: 800,
    actions: [
      {
        text: t('settingsManager.cancel'),
        action: () => closeDialog(),
      },
      {
        text: t('settingsManager.resetSettings'),
        action: () => {
          localStorage.clear()
          openSnackbar({ message: t('settingsManager.settingsResetSuccess'), variant: 'success' })
          closeDialog()
