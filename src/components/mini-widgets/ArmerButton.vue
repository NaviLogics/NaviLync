      ...
      <span class="inline-block font-extrabold align-middle unselectable">
        {{
          vehicleStore.isArmed === undefined
            ? '...'
            : vehicleStore.isArmed
            ? $t('miniWidgets.armerButton.armed')
            : $t('miniWidgets.armerButton.disarmed')
        }}
      </span>
    </div>
  </button>
...
const arm = async (): Promise<void> => {
  try {
    await slideToConfirm({ command: t('miniWidgets.armerButton.arm') }, canByPassCategory(EventCategory.ARM))
    await vehicleStore.arm()
  } catch (error) {
    openSnackbar({
      message: `${t('miniWidgets.armerButton.armFailed')}: ${(error as Error).message}`,
      variant: 'error',
      duration: 3000,
    })
  }
...
const disarm = async (): Promise<void> => {
  try {
    await slideToConfirm({ command: t('miniWidgets.armerButton.disarm') }, canByPassCategory(EventCategory.DISARM))
    await vehicleStore.disarm()
  } catch (error) {
    openSnackbar({
      message: `${t('miniWidgets.armerButton.disarmFailed')}: ${(error as Error).message}`,
      variant: 'error',
      duration: 3000,
    })
  }
