        ...
        <div class="flex flex-col gap-y-1 p-2 overflow-y-auto max-h-[calc(100vh-150px)]">
          ...
          >
            <div class="flex justify-between items-center">
              <span class="text-[11px] font-bold opacity-80">{{ $t('missionPlanning.vertex') }} {{ index + 1 }}</span>
              <v-btn
                icon="mdi-delete"
                size="x-small"
                variant="text"
            ...
            <div class="flex flex-col justify-center w-full items-center text-white">
              <div class="flex w-full gap-x-4 my-[2px] justify-between text-[11px] text-center">
                <p class="w-[50px] text-start opacity-70">{{ $t('missionPlanning.latitude') }}:</p>
                <input
                  type="number"
                  step="0.000001"
                  :value="vertex.lat"
              ...
              <div class="flex w-full gap-x-4 my-[2px] justify-between text-[11px] text-center">
                <p class="w-[50px] text-start opacity-70">{{ $t('missionPlanning.longitude') }}:</p>
                <input
                  type="number"
                  step="0.000001"
                  :value="vertex.lng"
