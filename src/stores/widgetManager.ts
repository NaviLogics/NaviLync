...
export const useWidgetManagerStore = defineStore('widget-manager', () => {
  ...
  const removeElementFromCustomWidget = (elementHash: string): void => {
    ...
    if (!customWidgetElement) {
      throw new Error(i18n.global.t('stores.widgetManager.elementNotFound'))
    }
  ...
  const loadWidgetFromFile = (widgetHash: string, loadedWidget: Widget): void => {
    ...
    openSnackbar({
      variant: 'success',
      message: i18n.global.t('stores.widgetManager.widgetLoadedSuccess'),
      duration: 3000,
    })
  }
  ...
  const importProfile = (e: Event): void => {
    ...
    reader.onload = (event: Event) => {
      ...
      try {
        ...
        showDialog({ variant: 'error', message: i18n.global.t('stores.widgetManager.invalidProfileFile', { error }) })
        return
      }
  ...
  function renameView(view: View, name: string): void {
    const index = currentProfile.value.views.indexOf(view)
    if (name.length === 0) {
      showDialog({ variant: 'error', message: i18n.global.t('stores.widgetManager.viewNameBlank'), timer: 2000 })
      return
    }
  ...
  const importView = (e: Event): void => {
    ...
    reader.onload = (event: Event) => {
      ...
      try {
        ...
        showDialog({ variant: 'error', message: i18n.global.t('stores.widgetManager.invalidViewFile', { error }) })
        return
      }
