<template>
  <BaseConfigurationView>
    <template #title>{{ $t('missionConfig.title') }}</template>
    <template #content>
      <div
      ...
      >
        <div class="grid grid-cols-3 gap-x-4 mb-4">
          <v-switch
            ...
            :label="$t('missionConfig.enablePreArmChecklist')"
            color="white"
            hide-details
            base-color="#FFFFFF33"
          ...
          <v-switch
            ...
            :label="$t('missionConfig.enableSlideToConfirm')"
            color="white"
            hide-details
            base-color="#FFFFFF33"
            class="mt-2 -mb-2 ml-3"
          ...
          <v-switch
            ...
            :label="$t('missionConfig.autoSwitchToFlightMode')"
            color="white"
            hide-details
            base-color="#FFFFFF33"
            class="mt-2 -mb-2 ml-3"
          ...
          <v-switch
            ...
            :label="$t('missionConfig.showMissionCreationChecklist')"
            color="white"
            hide-details
            base-color="#FFFFFF33"
            class="mt-2 -mb-2 ml-3"
          ...
          <v-switch
            ...
            :label="$t('missionConfig.showCoordinateGrid')"
            color="white"
            hide-details
            base-color="#FFFFFF33"
            class="mt-2 -mb-2 ml-3"
        ...
        <ExpansiblePanel no-bottom-divider :is-expanded="!interfaceStore.isOnPhoneScreen">
          <template #title>{{ $t('missionConfig.enableConfirmationCategories') }}</template>
          <template #info>
            {{ $t('missionConfig.confirmationCategoriesInfo') }}
          </template>
          <template #content>
            <div class="flex flex-wrap items-center justify-start">
              <div
                ...
                v-show="
                  ...
                  (category !== EventCategory.ALT_CHANGE && category !== EventCategory.GOTO)
                "
                :key="category"
                class="min-w-[100px]"
              ...
              >
                <v-checkbox
                  ...
                  :label="$t(`confirmationCategories.${categoryKey(category)}`)"
                  hide-details
                ></v-checkbox>
              </div>
            </div>
        ...
        >
          <template #title>{{ $t('missionConfig.mapOptions') }}</template>
          <template #info>
            <strong>{{ $t('missionConfig.defaultMapPosition') }}:</strong>
            {{ $t('missionConfig.defaultMapPositionInfo') }} <br />
            <strong>{{ $t('missionConfig.maxVehiclePositionUpdateRate') }}:</strong>
            {{ $t('missionConfig.maxVehiclePositionUpdateRateInfo') }}
          </template>
          <template #content>
            <div class="flex flex-wrap gap-4 px-4 pb-4">
              <p class="w-full text-md">{{ $t('missionConfig.defaultMapPosition') }}</p>
              <div class="flex w-[70%] justify-around items-center">
                <div class="flex flex-col max-w-[9rem]">
                  <p class="text-sm text-slate-200 mb-2">{{ $t('missionConfig.latitude') }}</p>
                  <input
                ...
                <div class="flex flex-col max-w-[9rem]">
                  <p class="text-sm text-slate-200 mb-2 ml-4">{{ $t('missionConfig.longitude') }}</p>
                  <input
                ...
                <div class="flex flex-col max-w-[9rem]">
                  <p class="text-sm text-slate-200 mb-2 ml-4">{{ $t('missionConfig.zoomLevel') }}</p>
                  <input
                ...
                <v-btn class="mt-7 bg-[#FFFFFF22]" variant="plain" size="small" @click="saveMapPosition">{{
                  $t('missionConfig.save')
                }}</v-btn>
              </div>
              <div class="flex w-[63%] justify-between items-center mt-4">
                <p class="w-full text-md">{{ $t('missionConfig.maxVehiclePositionUpdateRate') }}</p>
...
/**
 * Map EventCategory enum values to translation keys
 * @param {string} category - The EventCategory value
 * @returns {string} The translation key
 */
const categoryKeyMap: Record<string, string> = {
  [EventCategory.ARM]: 'arm',
  [EventCategory.DISARM]: 'disarm',
  [EventCategory.TAKEOFF]: 'takeoff',
  [EventCategory.LAND]: 'land',
  [EventCategory.ALT_CHANGE]: 'altitudeChange',
  [EventCategory.GOTO]: 'goto',
}
const categoryKey = (category: string): string => categoryKeyMap[category] ?? category
