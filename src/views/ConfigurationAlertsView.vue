<template>
  <BaseConfigurationView>
    <template #title>{{ t('alerts.title') }}</template>
    <template #content>
      <div
      ...
      >
        <ExpansiblePanel :is-expanded="!interfaceStore.isOnPhoneScreen" no-top-divider>
          <template #title>{{ t('alerts.voiceAlerts.title') }}</template>
          <template #info>
            {{ t('alerts.voiceAlerts.info') }}
          </template>
          <template #content>
            <div class="flex justify-between">
              <v-switch
                v-model="alertStore.enableVoiceAlerts"
                :label="t('alerts.voiceAlerts.enable')"
                color="white"
                class="ml-3"
              />
              <v-slider
                ...
                step="0.05"
                hide-details
                :label="t('alerts.voiceAlerts.volume')"
                color="white"
                class="max-w-[300px]"
                :disabled="!alertStore.enableVoiceAlerts"
              />
            </div>
            <span class="text-sm font-medium mt-4">{{ t('alerts.voiceAlerts.levels') }}</span>
            <div class="flex flex-wrap items-center justify-start">
              ...
              >
                <v-checkbox
                  ...
                  :label="t(`alertLevels.${enabledLevel.level}`)"
                  hide-details
                  color="white"
                />
              </div>
            </div>
            <span class="text-sm font-medium mt-4">{{ t('alerts.voiceAlerts.voice') }}</span>
            <Dropdown
